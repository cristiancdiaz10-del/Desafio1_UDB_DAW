...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (usuarioActual) {
        ...
        const lblTopbar = document.getElementById('lbl-admin-topbar');

        if (lblSidebar) {
            lblSidebar.innerText = usuarioActual.nombreCompleto;
        }

        if (lblTopbar) {
            lblTopbar.innerText = usuarioActual.nombreCompleto;
        }
    }
    ...
    if (tbody) {
        ...
        if (aspirantesFiltrados.length === 0) {
            ...
            aspirantesFiltrados.forEach(asp => {
                ...
                tr.innerHTML = `
                    <td class="fw-semibold">${asp.nombreCompleto || 'Sin nombre'}</td>
                    <td>${asp.curso || 'Sin curso'}</td>
