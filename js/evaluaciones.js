...
seleccionarAspirante.addEventListener("change", function () {
    ...
    document.getElementById("telefonoCandidato").textContent =
        aspiranteSeleccionado.telefono || "--";
...
// Cargar automáticamente el aspirante seleccionado desde el dashboard
var aspiranteGuardado =
    JSON.parse(localStorage.getItem("aspiranteEvaluar"));

if (aspiranteGuardado) {

    seleccionarAspirante.value =
        aspiranteGuardado.email;

    seleccionarAspirante.dispatchEvent(
        new Event("change")
    );
}
...
function avanzarAspirante(etapa, estado) {
    ...
    var indice = usuarios.findIndex(function(usuario) {
        return usuario.email === emailAspirante;
    });
...
document.getElementById("btnAprobar").addEventListener("click", function() {
    ...
    var aspirante = aspirantes.find(function(usuario) {
        return usuario.email === seleccionarAspirante.value;
    });
    ...
    if (!aspirante.cvAdjuntado || !aspirante.videoAdjuntado) {
        alert("El aspirante debe tener el CV y el vídeo adjuntos.");
        return;
    }
    ...
    if (avanzarAspirante(4, "En proceso")) {
        ...
        alert(
            "Expediente validado correctamente. El aspirante ha avanzado a la etapa de Entrevista."
        );
    }
});
...
document.getElementById("btnCompletarEntrevista").addEventListener("click", function() {
    ...
    if (avanzarAspirante(5, "En proceso")) {
        ...
        alert(
            "Entrevista completada correctamente. El aspirante ha avanzado a la etapa de Psicométricas."
        );
    }
});
...
document.getElementById("btnCompletarPsicometrica").addEventListener("click", function() {
    ...
    if (avanzarAspirante(6, "En proceso")) {
        ...
        alert(
            "Evaluación psicométrica completada correctamente. El aspirante ha avanzado a la etapa Técnica."
        );
    }
});

    // Aprobar candidato al finalizar la etapa Técnica
document.getElementById("btnAprobarFinal").addEventListener("click", function() {
    ...
    usuarios[indice].resultadoFinal = "Aprobado";
    usuarios[indice].estado = "Aprobado";
    ...
    alert("El aspirante ha sido aprobado exitosamente.");
});


// Denegar candidato al finalizar la etapa Técnica
document.getElementById("btnDenegarFinal").addEventListener("click", function() {
    ...
    usuarios[indice].resultadoFinal = "Denegado";
    usuarios[indice].estado = "Denegado";
    ...
    alert("El aspirante ha sido denegado.");
});
